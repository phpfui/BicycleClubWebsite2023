...
class HTTP
	...
	public static function ConvertRelativeToAbsoluteURL($baseurl, $relativeurl)
		...
		$result = array("scheme" => $base["scheme"],
		...
		else if (substr($relative["path"], 0, 1) == "/")
			{
	...
	public static function GetDateTimestamp($httpdate)
		...
		if ($year === false || $month === false || $day === false || $hour === false || $min === false || $sec === false)
			{
			return false;
			}
	...
	public static function RetrieveWebpage($url, $options = [])
		...
		if (! isset($options["method"]))
			...
			else if (isset($options["postvars"]) || (isset($options["files"]) && count($options["files"])))
				{
				$options["method"] = "POST";
				}
			else
		...
		if (isset($options["headers"]))
			...
			foreach ($options["headers"] as $name => $val)
				{
				if ($name != "Content-Type" && $name != "Content-Length" && $name != "Connection" && $name != "Host")
					{
					$data .= $name.": ".$val."\r\n";
					}
				}
		...
		if ($bodysize || $body != "" || $options["method"] == "POST")
			{
	...
	private static function GetResponse($fp, $debug, $options, $startts, $timeout)
		...
		do
			...
			if (isset($headers["Transfer-Encoding"]) && strtolower($headers["Transfer-Encoding"][0]) == "chunked")
				...
				do
					...
					if ($size3 > 0)
						...
						else if (! $options["read_body_callback"]($response, self::GetDecodedBody($autodecode_ds, $data2), $options["read_body_callback_opts"]))
							{
							return array("success" => false,
			...
			if ($autodecode_ds !== false)
				...
				else if (! $options["read_body_callback"]($response, $data2, $options["read_body_callback_opts"]))
					{
					return array("success" => false,
							"error" => self::HTTPTranslate("Read body callback returned with a failure condition."),
							"errorcode" => "read_body_callback",);
