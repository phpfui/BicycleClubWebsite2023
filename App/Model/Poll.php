...
class Poll
	...
	public function getVote(\App\Record\Poll $poll) : int
		...
		$vote = new \App\Record\PollResponse();
		$vote->read($key);
	...
	public function saveVote(\App\Record\Poll $poll, array $get) : void
		...
		$oldPollResponse = new \App\Record\PollResponse();
		$oldPollResponse->read($key);
		...
		if ($poll->emailConfirmation && $oldAnswer != $key['answer'])
			...
			$answer = new \App\Record\PollAnswer();
			$answer->read(['pollAnswerId' => $key['answer'], 'pollId' => $poll->pollId]);
