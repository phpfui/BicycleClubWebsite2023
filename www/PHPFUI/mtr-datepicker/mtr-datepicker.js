...
function MtrDatepicker (inputConfig) {
  ...
  var setMinutes = function (input, preventAnimation) {
			console.log('in setMinutes='+input);
  ...
  var updateInputSlider = function (reference, newValue, preventAnimation) {
			console.log('updateInputSlider id: '+reference+' value:'+newValue);
    var element = byId(reference);
    if (!element) {
			console.log('updateInputSlider id not found '+reference);
      return;
    }
  ...
  function byId (selector) {
    var temp = document.getElementById(selector);
		if (! temp) {
			console.log('id not found: '+selector);
		}
		return temp;
  }
