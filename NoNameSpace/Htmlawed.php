...
function htmLawed($t, $C = 1, $S = []) {
	...
	foreach (\explode(';', \trim(\str_replace([' ', "\t", "\r", "\n"], '', $x), ';')) as $v) {
		$x = $x2 = null;
		[$x, $x2] = \explode(':', $v, 2);
...
function hl_bal($t, $do = 1, $in = 'div') {
	// balance tags
// by content
	$cB = ['form' => 1, 'map' => 1, 'noscript' => 1]; // Block
	...
	$cF = ['a' => 1, 'article' => 1, 'aside' => 1, 'audio' => 1, 'button' => 1, 'blockquote' => 1, 'canvas' => 1, 'del' => 1, 'details' => 1, 'div' => 1, 'dd' => 1, 'fieldset' => 1, 'figure' => 1, 'footer' => 1, 'header' => 1, 'iframe' => 1, 'ins' => 1, 'li' => 1, 'main' => 1, 'menu' => 1, 'nav' => 1, 'noscript' => 1, 'object' => 1, 'section' => 1, 'style' => 1, 'td' => 1, 'th' => 1, 'video' => 1]; // Flow; later context-wise dynamic move of ins & del to $cI
...
// block/inline type; a/ins/del both type; #pcdata: text
	...
	for ($i = -1, $ci = \count($t); ++$i < $ci;) {
		...
		if (isset($x[0])) {
			if (\strlen(\trim($x)) && (($ql && isset($cB[$p])) || (isset($cB[$in]) && ! $ql))) {
				echo '<div>', $x, '</div>';
...
function hl_spec($t) {
	...
	$t = \str_replace(["\t", "\r", "\n", ' '], '', \preg_replace_callback('/"(?>(`.|[^"])*)"/sm', 'hl_aux1', \trim($t)));
...
function hl_tag($t) {
		...
		switch ($mode) {
			...
			case 2: // Val
				if (\preg_match('`^((?:"[^"]*")|(?:\'[^\']*\')|(?:\s*[^\s"\']+))(.*)`', $a, $m)) {
					...
					$w = 1;
					$mode = 0;
					$aA[$nm] = \trim(\str_replace('<', '&lt;', ('"' == $m[0] || '\'' == $m[0]) ? \substr($m, 1, -1) : $m));
				}
	...
	foreach ($aA as $k => $v) {
		if (((isset($d['*']) ? isset($d[$k]) : ! isset($d[$k])) && (isset($aN[$k][$e]) || isset($aNU[$k]) || (isset($aNO[$k]) && ! isset($d['on*'])) || (isset($aNA[$k]) && ! isset($d['aria*'])) || (! isset($d['data*']) && \preg_match('`data-((?!xml)[^:]+$)`', $k))) && ! isset($rl['n'][$k]) && ! isset($rl['n']['*'])) || isset($rl[$k])) {
			if (isset($aNE[$k])) {
				$v = $k;
...
function hl_tag2(&$e, &$a, $t = 1) {
	...
	if ('font' == $e) {
		...
		while (\preg_match('`(^|\s)(color|size)\s*=\s*(\'|")?(.+?)(\\3|\s|$)`i', $a, $m)) {
			$a = \str_replace($m[0], ' ', $a);
			$a2 .= 'color' == \strtolower($m[2]) ? (' color: ' . \str_replace(['"', ';', ':'], '\'', \trim($m[4])) . ';') : (isset($fs[($m = \trim($m[4]))]) ? (' font-size: ' . $fs[$m] . ';') : '');
		}

		while (\preg_match('`(^|\s)face\s*=\s*(\'|")?([^=]+?)\\2`i', $a, $m) || \preg_match('`(^|\s)face\s*=(\s*)(\S+)`i', $a, $m)) {
			$a = \str_replace($m[0], ' ', $a);
			$a2 .= ' font-family: ' . \str_replace(['"', ';', ':'], '\'', \trim($m[3])) . ';';
		}
		$e = 'span';
