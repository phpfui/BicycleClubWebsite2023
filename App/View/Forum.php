...
class Forum
	...
	public function home(\App\Record\Forum $forum, bool $showSubscription = false) : \PHPFUI\Container
		...
		if (! $forum->closed)
			...
			$member = new \App\Record\ForumMember();
			$member->read($key);
			...
			if (isset($_POST['action']))
				...
				switch ($_POST['action'])
					...
					case 'deleteMessage':
						if ($this->page->isAuthorized('Delete Forum Message'))
							{
							$forumMesage = new \App\Record\ForumMessage((int)$_POST['forumMessageId']);
							$forumMesage->delete();
					...
					case 'deleteMember':
						$forumMember = new \App\Record\ForumMember();
						$forumMember->read(['forumId' => (int)$_POST['forumId'], 'memberId' => (int)$_POST['memberId']]);
						$forumMember->delete();
