...
class Member extends \PHPFUI\ORM\Table
{
	...
	public function findByName(array $names, bool $currentMembers = true) : \PHPFUI\ORM\ArrayCursor
		{
		$sql = 'select m.firstName,m.lastName,m.memberId,m.email,m.showNothing,s.*
			from member m
			left join membership s on m.membershipId=s.membershipId where ';
		$input = [];
		...
		$sql .= ' order by firstName,lastName,memberId';
	 ...
	 */
	public function getEmailableMembers(bool $all, bool $current, int $monthsPast = 0, int $monthsNew = 0, array $categories = [], string $extra = '') : \PHPFUI\ORM\RecordCursor
		{
	...
	public function getNewRideInterests(int $categoryId) : \PHPFUI\ORM\ArrayCursor
		...
		$sql = 'SELECT distinct m.firstName,m.lastName,m.email,m.memberId from member m left join memberCategory c on m.memberId=c.memberId ' .
			'left join membership s on s.membershipId=m.membershipId ' .
			'where m.newRideEmail > 0 and c.categoryId=? and m.email like "%@%" and s.expires>=? order by m.memberId';
	...
	public static function getVolunteersForEvents(array $events) : \PHPFUI\ORM\ArrayCursor
		...
		$sql = 'select m.memberId,m.firstName,m.lastName,m.email from member m
			...
			where je.jobEventId in (' . \implode(',', \array_fill(0, \count($events), '?')) . ') group by m.memberId order by m.memberId';
	...
	public function getVolunteersForJob(\App\Record\Job $job) : \PHPFUI\ORM\ArrayCursor
		{
		$sql = 'select m.memberId,m.firstName,m.lastName,m.email from member m left join volunteerJobShift vjs on vjs.memberId=m.memberId left join job j on j.jobId=vjs.jobId left join jobEvent je on je.jobEventId=j.jobEventId where j.jobId=? group by m.memberId order by m.memberId';
