...
class API
	...
	public function list(\App\Table\OauthUser $table) : \PHPFUI\Container
		...
		if (\count($table))
			...
			$view->addCustomColumn('edit', static function(array $row) { return new \PHPFUI\FAIcon('fas', 'edit', '/System/API/edit/' . $row['oauthUserId']);});
			$view->addCustomColumn('passwordSet', static function(array $row) { return $row['password'] ? 'Yes' : 'No';});
			$view->addCustomColumn('permissions', static function(array $row) { return new \PHPFUI\FAIcon('fas', 'check-square', '/System/API/permissions/' . $row['oauthUserId']);});
	...
	public function serverSettings() : \PHPFUI\Container
		{
		$container = new \PHPFUI\Container();
		...
		if (\App\Model\Session::checkCSRF() && ($_POST['submit'] ?? '') == $submit->getText())
			...
			$encryptionKeyName = 'APIEncryptionKey';
			$key = $settingTable->value($encryptionKeyName);

			if (empty($key))
				{
				$key = \Defuse\Crypto\Key::createNewRandomKey();
				$settingTable->save($encryptionKeyName, $key->saveToAsciiSafeString());
				}

			if (! empty($_POST['privateKey']))
				{
				\file_put_contents(\App\Model\API::getPrivateKeyPath(), $_POST['privateKey']);
				}

			if (! empty($_POST['publicKey']))
				{
				\file_put_contents(\App\Model\API::getPublicKeyPath(), $_POST['publicKey']);
				}
			$this->page->redirect();
		...
		if (\file_exists(\App\Model\API::getPrivateKeyPath()))
			{
			$ul->addItem(new \PHPFUI\ListItem('API Private Key has been uploaded'));
			}

		if (\file_exists(\App\Model\API::getPublicKeyPath()))
			{
			$ul->addItem(new \PHPFUI\ListItem('API Public Key has been uploaded'));
			}
		...
		if (\count($ul))
			...
			{
			$callout->add('The Oauth2 server needs public and private keys to operate. Use these Linux commands to generate keys, then upload them below.');
			$code = new \PHPFUI\HTML5Element('code');
			$code->addClass('code-block');
			$code->add('openssl genrsa -out private.key 2048');
			$code->add('openssl rsa -in private.key -pubout -out public.key');
			$callout->add($code);
			$callout->add('Paste the contents of private.key and public.key below.');
			}
		...
		if (! \file_exists(\App\Model\API::getPrivateKeyPath()))
			{
			$ul->addItem(new \PHPFUI\ListItem('API Private Key is missing'));
			}

		if (! \file_exists(\App\Model\API::getPublicKeyPath()))
			{
			$ul->addItem(new \PHPFUI\ListItem('API Public Key is missing'));
			}
		...
		$fieldSet = new \PHPFUI\FieldSet('Server Keys');
		$privateKey = new \PHPFUI\Input\TextArea('privateKey', 'Private Key Text');
		$privateKey->setRequired();
		$fieldSet->add($privateKey);
		$publicKey = new \PHPFUI\Input\TextArea('publicKey', 'Public Key Text');
		$publicKey->setRequired();
		$fieldSet->add($publicKey);
