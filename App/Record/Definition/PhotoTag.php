<?php

namespace App\Record\Definition;

/**
 * Autogenerated. Do not modify. Modify SQL table, then run oneOffScripts\generateCRUD.php table_name
 *
 * @property int $frontToBack MySQL type int(11)
 * @property int $leftToRight MySQL type int(11)
 * @property ?int $memberId MySQL type int(11)
 * @property \App\Record\Member $member related record
 * @property int $photoId MySQL type int(11)
 * @property \App\Record\Photo $photo related record
 * @property string $photoTag MySQL type varchar(100)
 * @property int $photoTagId MySQL type int(11)
 * @property int $taggerId MySQL type int(11)
 * @property \App\Record\Tagger $tagger related record
 */
abstract class PhotoTag extends \PHPFUI\ORM\Record
	{
	protected static bool $autoIncrement = true;

	/** @var array<string, array<mixed>> */
	protected static array $fields = [
		// MYSQL_TYPE, PHP_TYPE, LENGTH, NULL, DEFAULT, KEY
		'frontToBack' => ['int(11)', 'int', 11, false, null, false, ],
		'leftToRight' => ['int(11)', 'int', 11, false, null, false, ],
		'memberId' => ['int(11)', 'int', 11, true, null, true, ],
		'photoId' => ['int(11)', 'int', 11, false, null, true, ],
		'photoTag' => ['varchar(100)', 'string', 100, false, '', false, ],
		'photoTagId' => ['int(11)', 'int', 11, false, null, true, ],
		'taggerId' => ['int(11)', 'int', 11, false, null, false, ],
	];

	/** @var array<string, true> */
	protected static array $primaryKeys = ['photoTagId' => true, ];

	protected static string $table = 'photoTag';
	}
