...
class Client
{
	...
	public function __construct($key, $app_identifier = null, $proxy = null) {
		...
		$this->options = [
			...
			CURLOPT_USERAGENT => \implode(' ', \array_filter([self::userAgent(), $app_identifier])),
		];
	...
	public function request($method, $url, $body = null) {
		...
		for ($retries = self::RETRY_COUNT; $retries >= 0; $retries--) {
			...
			$url = 'https:' == \strtolower(\substr($url, 0, 6)) ? $url : self::API_ENDPOINT . $url;
			...
			if (\is_string($response)) {
				...
				if (isset($headers['compression-count'])) {
					Tinify::setCompressionCount((int)($headers['compression-count']));
				}

				if ($status >= 200 && $status <= 299) {
					return (object)['body' => $responseBody, 'headers' => $headers];
				}
	...
	protected static function parseHeaders($headers) {
		...
		foreach ($headers as $header) {
			...
			if (2 === \count($split)) {
				$res[\strtolower($split[0])] = \trim($split[1]);
			}
		}
