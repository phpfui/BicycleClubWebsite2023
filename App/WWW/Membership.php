...
class Membership extends \App\View\WWWBase implements \PHPFUI\Interfaces\NanoClass
	...
	public function roster(string $field = '', string $select = '', int $offset = 0) : void
		...
		if ($this->page->addHeader('Club Roster'))
			{
			$this->page->addPageContent(new \PHPFUI\Header('Members with privacy settings enabled are not shown', 6));
	...
	public function socialMedia() : void
		...
		if ($this->page->addHeader('Social Media Exceptions'))
			...
			$view->addCustomColumn('photo', static fn (array $member) : string => $memberView->getImageIcon($member));
