...
class BaseClient
{
	public const DEFAULT_REGION = 'us1';

	public const ENV_ACCOUNT_SID = 'TWILIO_ACCOUNT_SID';

	public const ENV_AUTH_TOKEN = 'TWILIO_AUTH_TOKEN';

	public const ENV_EDGE = 'TWILIO_EDGE';

	public const ENV_LOG = 'TWILIO_LOG_LEVEL';

	public const ENV_REGION = 'TWILIO_REGION';
	...
	{
		if (null == $this->region && null == $this->edge) {
			return $uri;
		}
		...
		if (4 == \count($pieces)) { // product.region.twilio.com
			$newRegion = $newRegion ?: $pieces[1];
		} elseif (5 == \count($pieces)) { // product.edge.region.twilio.com
			$newEdge = $newEdge ?: $pieces[1];
			$newRegion = $newRegion ?: $pieces[2];
		}

		if (null != $newEdge && null == $newRegion) {
	...
	public function request(
		...
		$logLevel = ('true' === \getenv('DEBUG_HTTP_TRAFFIC') ? 'debug' : $this->getLogLevel());
		...
		if ('debug' === $logLevel) {
			...
			foreach ($headers as $key => $value) {
				if (false === \strpos(\strtolower($key), 'authorization')) {
					\error_log("{$key}: {$value}");
				}
			}
