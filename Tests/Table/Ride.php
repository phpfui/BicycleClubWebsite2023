...
class Ride extends \PHPFUI\ORM\Table
	...
	public function unreportedRides() : \PHPFUI\ORM\RecordCursor
		...
		$sql = 'select * from ride r
			where r.rideStatus=0 and r.rideStatus=0 and r.rideDate<? and r.unaffiliated=0 and r.pending=0 order by r.rideDate desc, rideId limit 50';
	...
	public static function unreportedRidesOn(array $dates) : ?\PHPFUI\ORM\RecordCursor
		...
		$sql = 'select * from ride r
			...
			where r.unaffiliated=0 and r.rideStatus=0 and r.pending=0 and r.rideDate in ("' . $join . '") order by rideId';
	...
	public static function upcomingRides(int $limit = 0) : \PHPFUI\ORM\RecordCursor
		{
		$sql = 'select ride.*,pace.* from ride left join pace on pace.paceId=ride.paceId where rideDate >= ? and pending=0';
		$input = [\App\Tools\Date::todayString()];
