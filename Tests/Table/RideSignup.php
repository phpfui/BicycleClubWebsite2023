...
class RideSignup extends \PHPFUI\ORM\Table
{
	...
	public function getRidesForMember(\App\Record\Member $member, string $startDate = '2000-01-01', string $endDate = '2999-12-31') : \PHPFUI\ORM\DataObjectCursor
		{
		$sql = 'select * from rideSignup rs left join ride r on r.rideId=rs.rideId where rs.memberId=? and r.rideDate>=? and r.rideDate<=? order by r.rideDate desc, rs.memberId';
	...
	public static function getSignedUpByPermmission(\App\Record\Ride $ride, \App\Record\Permission $permission) : \PHPFUI\ORM\DataObjectCursor
		{
		$sql = 'SELECT * FROM member m left join userPermission u on u.memberId = m.memberId left join rideSignup r on r.memberId=m.memberId
			WHERE u.permissionGroup = ? and r.rideId = ?';
